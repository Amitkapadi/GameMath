...
//
// Modified by KarateSnoopy
// Copyright (c) 2013 Alexander Logger. All rights reserved.
// Original source from Alexander Logger from https://github.com/everbytes/BigMath
// Original license: https://github.com/everbytes/BigMath/blob/master/LICENSE.md
        ...
        public string ToString(string format, IFormatProvider formatProvider = null)
        {
            ...
            if (!string.IsNullOrEmpty(format))
            {
                ...
                if ((ch == 'x') || (ch == 'X'))
                {
                    ...
                    byte[] bytesA = BitConverter.GetBytes(_a);
                    byte[] bytesB = BitConverter.GetBytes(_b);
                    byte[] bytesC = BitConverter.GetBytes(_c);
                    byte[] bytesD = BitConverter.GetBytes(_d);
                    return 
                        ByteArrayToHexString(bytesA) + ' ' +
                        ByteArrayToHexString(bytesB) + ' ' +
                        ByteArrayToHexString(bytesC) + ' ' +
                        ByteArrayToHexString(bytesD);
                }
